...
* aor_dataset_generator.py: circuit 3 must use 2 Warburg params
  (Wo1_R, Wo1_T) — impedance.py's Wo takes exactly 2.
* transformer.py: MAX_PARAMS = 9 (circuit 3 has 9 parameters).

Usage
-----
Smoke test: python train_eis_gpt.py --samples-per-circuit 200 --epochs 5
Full run:   python train_eis_gpt.py --samples-per-circuit 2000 --epochs 50
Resume:     python train_eis_gpt.py --resume checkpoints/ckpt_ep20.pth ...
...
def reconstruct_spectrum(params, labels, freq):
    ...
    labels : (B,) circuit labels 0..4
    freq   : (B, N) frequencies in Hz
    Returns complex Z (B, N).
    """
...
def build_tensors(records):
    ...
    p_mask   = torch.zeros(n, MAX_PARAMS)
...
# ──────────────────────────────────────────────────────────────────────
# Loss = classification NLL + masked Gaussian NLL + physics penalties
# ──────────────────────────────────────────────────────────────────────

def compute_loss(model, out, freq, z_real, z_imag, labels,
                 p_target, p_mask, lam_param, lam_physics,
                 sigma_floor=1e-2):
    """lam_physics should already include any warmup scaling."""
    loss_cls = F.nll_loss(out["circuit_logprobs"], labels)

    mu, sigma = out["param_mu"], out["param_sigma"]
    sigma = sigma.clamp(min=sigma_floor)  # floor prevents NLL gradient blow-up
    nll = 0.5 * (((p_target - mu) / sigma) ** 2 + 2 * torch.log(sigma))
    loss_param = (nll * p_mask).sum() / p_mask.sum().clamp(min=1)

    # Physics-informed term: decode mu -> spectrum -> physics penalties.
    # mu is in log10-space; clamp before exponentiation for stability.
...
def main():
    ...
    ap.add_argument("--sigma-floor",         type=float, default=1e-2,
                    help="minimum predicted sigma in log10-decades")
    ...
    print(f"Generating dataset ({args.samples_per_circuit}/circuit)...")
    ...
    if n_classes < 5:
        print("WARNING: fewer than 5 classes generated — check "
              "AOR_CIRCUIT_LIBRARY (Wo1 must have exactly 2 parameters).")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        model.train()
        # linear warmup keeps the raw-10**mu physics path stable early on
        ...
        for freq, zr, zi, lab, pt, pm in train_loader:
            ...
            if not torch.isfinite(loss):
                optimizer.zero_grad()
                continue  # skip pathological batch instead of poisoning weights
            optimizer.zero_grad()
            loss.backward()
            gnorm = torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            if not torch.isfinite(gnorm):
                optimizer.zero_grad()
                continue  # inf/nan gradients -> skip step, keep weights clean
            optimizer.step()
        ...
        with torch.no_grad():
            for freq, zr, zi, lab, pt, pm in val_loader:
                ...
                lab,  pt, pm = lab.to(device),  pt.to(device), pm.to(device)
                out  = model(freq, zr, zi)
                loss, *_ = compute_loss(model, out, freq, zr, zi, lab, pt,
                                        pm, args.lambda_param, lam_phys,
                                        args.sigma_floor)
        ...
        score = acc - 0.1 * rmse             # combined criterion
    ...
    print(f"Weights: {out_path} | Checkpoints: {ckpt_dir}/")
