...
class ECSACalculator:
    ...
    def _split_scans(
        ...
        against unequal numbers of points in each half.
    ...
    def method_a_hupd(
        ...
        Args:
            ...
            q_ref      : reference charge density (uC/cm2); defaults to Q_H_PT
    ...
    def method_c_cdl(
        ...
        Args:
            ...
            cs_mF_cm2       : specific capacitance (mF/cm2); defaults to CS_CARBON
        ...
        for i, (pot, cur) in enumerate(zip(potentials_list, currents_list)):
            ...
            j_a = float(np.interp(v_mid, fwd_pot, fwd_cur))
            j_c = float(np.interp(v_mid, bwd_pot[::-1], bwd_cur[::-1]))

            # Normalise to geometric area -> A/cm2
